...
class RateLimitService:
    ...
    def generate_fingerprint(self, request: Request) -> str:
        ...
        guest_token = request.headers.get("x-guest-token", request.cookies.get("guest_token", ""))
    ...
    def check_rate_limit(self, request: Request):
        ...
        if current and int(current) >= self.MAX_QUERIES:
            raise HTTPException(status_code=429, detail="Rate limit exceeded. Maximum 30 queries per week.")
